...
import { IFilm, FilmService, FilmDetailsComponent, FilmListComponent, FilmNewComponent } from '../';
  ...
  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [FilmListComponent, ListComponent],
        imports: [TestingModule],
        providers: [FilmService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/film/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
