...
import { IStore, StoreService, StoreDetailsComponent, StoreListComponent, StoreNewComponent } from '../';
  ...
  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [StoreListComponent, ListComponent],
        imports: [TestingModule],
        providers: [StoreService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/store/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
