...
import { ICity, CityService, CityDetailsComponent, CityListComponent, CityNewComponent } from '../';
  ...
  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [CityListComponent, ListComponent],
        imports: [TestingModule],
        providers: [CityService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/city/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
