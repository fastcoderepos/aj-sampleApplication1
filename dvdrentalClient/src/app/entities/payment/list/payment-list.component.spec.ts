...
import { IPayment, PaymentService, PaymentDetailsComponent, PaymentListComponent, PaymentNewComponent } from '../';
...
describe('PaymentListComponent', () => {
  ...
  let constData: IPayment[] = [
    ...
    },
    {
      ...
      paymentId: 2,
      customerId: 2,
      customerDescriptiveField: 'firstName2',
      rentalId: 2,
      rentalDescriptiveField: 2,
      staffId: 2,
      staffDescriptiveField: 'firstName2',
    },
  ];
  let data: IPayment[] = [...constData];

  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [PaymentListComponent, ListComponent],
        imports: [TestingModule],
        providers: [PaymentService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/payment/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
