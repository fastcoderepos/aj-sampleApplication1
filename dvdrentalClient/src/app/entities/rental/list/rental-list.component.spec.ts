...
import { IRental, RentalService, RentalDetailsComponent, RentalListComponent, RentalNewComponent } from '../';
...
describe('RentalListComponent', () => {
  ...
  let constData: IRental[] = [
    ...
    },
    {
      ...
      inventoryId: 2,
      inventoryDescriptiveField: 2,
      staffId: 2,
      staffDescriptiveField: 'firstName2',
    },
  ];
  let data: IRental[] = [...constData];

  describe('Unit tests', () => {
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        declarations: [RentalListComponent, ListComponent],
        imports: [TestingModule],
        providers: [RentalService, ChangeDetectorRef],
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(decodeURIComponent(location.path())).toBe(
        `/rental/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`
      );
    });
