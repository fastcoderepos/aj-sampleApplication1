...
import com.fastcode.dvdrental.domain.core.city.QCityEntity;
    ...
    protected BooleanBuilder searchKeyValuePair(
        ...
        for (Map.Entry<String, SearchFields> details : map.entrySet()) {
            ...
            if (details.getKey().replace("%20", "").trim().equals("country")) {
                if (details.getValue().getOperator().equals("contains")) builder.and(
                    city.country.country.likeIgnoreCase("%" + details.getValue().getSearchValue() + "%")
                ); else if (details.getValue().getOperator().equals("equals")) builder.and(
                    city.country.country.eq(details.getValue().getSearchValue())
                ); else if (details.getValue().getOperator().equals("notEqual")) builder.and(
                    city.country.country.ne(details.getValue().getSearchValue())
                );
            }
        }
