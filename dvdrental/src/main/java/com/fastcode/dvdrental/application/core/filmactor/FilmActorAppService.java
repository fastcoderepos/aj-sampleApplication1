    ...
    protected BooleanBuilder searchKeyValuePair(
        ...
        for (Map.Entry<String, SearchFields> details : map.entrySet()) {
            ...
            if (details.getKey().replace("%20", "").trim().equals("actor")) {
                if (details.getValue().getOperator().equals("contains")) builder.and(
                    filmActor.actor.firstName.likeIgnoreCase("%" + details.getValue().getSearchValue() + "%")
                ); else if (details.getValue().getOperator().equals("equals")) builder.and(
                    filmActor.actor.firstName.eq(details.getValue().getSearchValue())
                ); else if (details.getValue().getOperator().equals("notEqual")) builder.and(
                    filmActor.actor.firstName.ne(details.getValue().getSearchValue())
                );
            }
            if (details.getKey().replace("%20", "").trim().equals("film")) {
                if (details.getValue().getOperator().equals("contains")) builder.and(
                    filmActor.film.title.likeIgnoreCase("%" + details.getValue().getSearchValue() + "%")
                ); else if (details.getValue().getOperator().equals("equals")) builder.and(
                    filmActor.film.title.eq(details.getValue().getSearchValue())
                ); else if (details.getValue().getOperator().equals("notEqual")) builder.and(
                    filmActor.film.title.ne(details.getValue().getSearchValue())
                );
            }
        }
        ...
        for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
            ...
            if (joinCol != null && joinCol.getKey().equals("film")) {
                builder.and(filmActor.film.title.eq(joinCol.getValue()));
            }
        }
        return builder;
    }
